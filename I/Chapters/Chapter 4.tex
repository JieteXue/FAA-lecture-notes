...
\end{definitionenv}
\begin{definitionenv}
    Let $\thicksim$ be an equivalent relation on $X$. For any $x\in X$,  we call the set $$[x]:=\{y\in X\mid y\thicksim x\}$$ the equivalent class of $x$ under $\thicksim$, we denote by $X/\thicksim$ the set $\{[x]\mid x\in X\}$ of all equivalent class.It is a subset of $\mathscr{P} (X)$.Moreover,  since $\forall x\in X, x\in [x] $,  one has $$X=\bigcup_{A\in X/\thicksim}A.$$
\end{definitionenv}
...
\newline
 ...
 \newline
\quad Increasing and decreasing functions are called \textbf{monotonic function},  strictly increasing and decreasing functions are called \textbf{strictly monotonic function}.
\end{definitionenv}
\begin{propositionenv}
...
\begin{definitionenv}
    ...
    \newline
    Denote by $A^\mathrm{u}$ the set of upper bounds of $A$ in $(X, \le)$.
    \newline
     Denote by $A^\mathrm{l}$ the set of lower bounds of $A$ in $(X, \le)$.

\end{definitionenv}
\begin{exampleenv}
    $\Omega=\{1, 2, 3\}, X=\mathscr{P} (\Omega).(X, \subseteq)$ forms a partially ordered set. Let $A=\{\{1\}, \{2\}, \{1, 2\}\}, A^\mathrm{u}=\{\{1, 2\}, \{1, 2, 3\}\}, A^\mathrm{l}=\{\varnothing\}$.
\end{exampleenv}
\begin{definitionenv}
    Let $(X, \le)$ be a partially ordered set , let $A$ be a subset of $X$.
    \newline
...
\end{definitionenv}
\begin{definitionenv}
    $A\subseteq Y\subseteq X$, let $A_Y^\mathrm{u}:=\{y\in Y\mid \forall a\in A, a\le y\}$ be the set of upper bounds of $A$ in $Y$.If $A_Y^\mathrm{u}$ has a least element, we call it the \textbf{supremum} of $A$ in $Y$,  denoted as $\mathrm{sup}_{(Y, \le)}A$,  if there's no ambiguity on $\le$ we can also write as $\mathrm{sup}_{Y}A$. Resp. \textbf{infimum}.
\end{definitionenv}
...
\begin{propositionenv}\label{proposition4.5.1}
    \quad
    \newline
    Let $(X, \le)$ be a partially ordered set $(A, Z, Y)\in \mathscr{P} (X)^3, A\subseteq Z\subseteq Y$.
    \newline
...
\begin{propositionenv}\label{proposition4.5.2}
    \quad
    \newline
    Let $(X, \le )$ be a partially ordered set, $(A, B, Y)\in \mathscr{P} (X)^3, A\subseteq B\subseteq Y$.
    \newline
...
\end{proofenv}
\begin{propositionenv}
    Let $(X, \le)$ be a partially ordered set , $Y\subseteq X, I$ be a set,  and $(A_i)_{i\in I}\in \mathscr{P} (Y)^I$. Let $\dis A=\bigcup _{i\in I}A_i$
    \newline
...
\begin{proofenv}
    ...
    \newline
    (ii) if $a\in I,  b\notin I, I\subseteq\interval[open right]{a}{b}=[a, b]\setminus\{b\}$. Let $x\in \interval[open right]{a}{b}$,  since $x<b$,  $x$ is not an upper bound of $I$. Hence $\exists y\in I,  x<y$. Note that $[a, y]\subseteq I$,  hence $x\in I$,  therefore $\interval[open right]{a}{b}\subseteq I$. Similarly ,  is $b\in I, a\notin I$,  then $\interval[open left]{a}{b}=I$.
    \newline
...
\begin{definitionenv}[Well-ordered]\label{4.7.1}
    Let $(X, \le)$ be a partially ordered set. If $\forall A\in \mathscr{P}(X), A\not=\varnothing\Rightarrow A$ has a least element, we say that $(X, \le)$ is a \textbf{well-ordered set}.
\end{definitionenv}
\begin{axiomenv}
...
\end{axiomenv}
\begin{exampleenv}
    Let $\Omega$ be a set, $X=\mathscr{P}(\Omega)$. Then $(X, \subseteq)$ is  order complete.
\end{exampleenv}
    \begin{proofenv}
        Let $Y\subseteq X$. Then 
        $$Y^{\mathrm{u}}=\{B\in\mathscr{P}(\Omega)\mid \forall A\in Y, A\subseteq B\}.$$
...
% Text Node
\draw (75, 124.4) node [anchor=north west][inner sep=0.75pt]    {$X\setminus C$};
% Text Node
\draw (350, 168.4) node [anchor=north west][inner sep=0.75pt]    {$Y\setminus f( C)$};
% Text Node
...
\end{theoremenv}
\begin{proofenv}
    Consider $\Phi:\mathscr{P}(X)\rightarrow\mathscr{P}(X), A\mapsto X\setminus g(Y\setminus f(A))$. If $(A, B)\in \mathscr{P}(X)^2$ such that $A\subseteq B$,  then $f(A)\subseteq f(B), Y\setminus f(A)\supseteq  Y\setminus f(B), g(Y\setminus f(A))\supseteq g(Y\setminus f(A)) , \Phi(A)\subseteq\Phi(B)$. So $\Phi$ is increasing. By Knaster-Tarski theorem,  $\exists C\in \mathscr{P}(X), C=\Phi(C)$. Then $h:X\rightarrow Y, h(x):=\left\{\begin{matrix}
 f(x), x\in C\\
g^{-1}(x), x\in X\setminus C
\end{matrix}\right.$
is a bijection.
\end{proofenv}
...
\begin{lemmaenv}
    ...
    \newline
    (1) Let $(A, B)\in \mathscr{P}(X)^2$,  if $A\subseteq B$,  then $B^\mathrm{u}\subseteq A^\mathrm{u}, B^\mathrm{l}\subseteq A^\mathrm{l}$.
    \newline
    (2) $\forall A\in \mathscr{P}(X), A\subseteq (A^\mathrm{u})^\mathrm{l}\cap (A^\mathrm{l})^\mathrm{u}$.
\end{lemmaenv}

\begin{theoremenv}[Dedekind-MacNeille]
    \quad
    \newline
    Let $(X, \le )$ be a partially ordered set. Let $\hat{X}:=\{A\in \mathscr{P} (X)\mid (A^\mathrm{u})^\mathrm{l}=A\}$.
\newline
(1) $(\hat{X}, \subseteq)$ is order complete.
\newline
(2) $\forall A\in \mathscr{P}(X), A^\mathrm{l}\in \hat{X}$.
\newline
...
$$A=\bigcup_{x\in A}\hat{x}=\sup_{(\mathscr{P}(X), \subseteq)}\{\hat{x}\mid x\in A\}=\sup_{(\hat{X}, \subseteq)}\{\hat{x}\mid x \in A\}.$$
\end{theoremenv}
\begin{remark}
    We've know that $(\mathscr{P}(X), \subseteq)$ is order complete. So for the sets not order complete,  we can build a relation between them to make it become order complete. And this theorem tell us how to do.
\end{remark}
\begin{proofenv}
    \quad
    \newline
    (1) Consider $\Phi :\mathscr{P}(X)\rightarrow\mathscr{P}(X), A\mapsto(A^\mathrm{u})^\mathrm{l}$. By the lemma,  $\Phi $  is increasing. Since $\mathscr{P}(X)$ is complete, and $\hat{X}$ is the set of fixed point of $\Phi$. By Knaster-Tarski fixed point theorem, $(\hat{X}, \subseteq)$ is order complete.
    \newline
    (2) Let $A\in \mathscr{P}(X)$,  we prove that $A^\mathrm{l}=((A^\mathrm{l})^\mathrm{u})^\mathrm{l}$. Since $A\subseteq (A^\mathrm{l})^\mathrm{u}$ (by the lemma),  $((A^\mathrm{l})^\mathrm{u})^\mathrm{l}\subseteq A^\mathrm{l}$,  by (2) of the lemma applied to $A^\mathrm{l}$. Hence $A^\mathrm{l}=((A^\mathrm{l})^\mathrm{u})^\mathrm{l}$
    \newline
    ...
    $$\inf _{(\mathscr{P}(X), \subseteq)}\{\hat{x}\mid x\in A^\mathrm{u}\}=\bigcap_{x\in A^\mathrm{u}}\hat{x}=\bigcap_{x\in A^\mathrm{u}}\{x\}^\mathrm{l}=(A^\mathrm{u})^\mathrm{l}=A.$$
    So it is equal to $\dis\inf_{(\hat{X}, \subseteq)}\{\hat{x}\mid x\in A ^\mathrm{u}\}$.
\end{proofenv}
\begin{remark}
    $\forall A\in \hat{X}, A=\{x\in X\mid \hat{x}\subseteq A\}, A^\mathrm{u}=\{x\in X\mid A\subseteq \hat{x}\}$.
\end{remark}
\begin{definitionenv}
...
\end{propositionenv}
\begin{proofenv}
    Assume that $I\setminus J\not=\varnothing$, take $x\in I\setminus J$, for any $y\in J$,  if $y\not\le x $, then $x<y$ and hence $x\in X_{<y}\subseteq J$, contradiction. Therefore $y\le x$. Then $y=x\in I$ or $y\in X_{<x}\subseteq I$.
\end{proofenv}
\begin{propositionenv}
    Let $(X, \le)$ be a well-ordered set. $I$ be an initial segment of $X$,  such that $I\not=X$. There is a unique $a\in X$ such that $I=X_{<a}$.
\end{propositionenv}
\begin{proofenv}
    $X\setminus I\not=\varnothing$ Let $a=\min (X\setminus I)$. By definition, $I\subseteq X_{<a}$. In fact,  $\forall y\in I$ if $y\not< a $,  then $a\le y$.
    Since $I$ is an initial segment $a\in I$,  contradiction.
    \newline
    Conversely,  if $x\in X_{<a}$,  then $x\notin X\setminus I$. Since otherwise $a\le x$. Therefore $x\in I$. Uniqueness, $\forall a\in X, a=\min(X\setminus X_{<a})=\min(X_\leq a)$. Hence $X_{<a}=X_{<b}\Rightarrow a=b$.
\end{proofenv}
\begin{propositionenv}
...
\begin{proofenv}
    \quad
    \newline
    Let $a\in I$. $\forall \lambda\in \Lambda, a\in I_\lambda$ and hence $X_{<a}\subseteq I_\lambda$. Therefore, $\dis X_{<a}\subseteq\bigcap_{\lambda\in\Lambda}I_\lambda=I$.
    \newline
    Let $b\in J$. Then $\exists \lambda_0\in \Lambda$ such that $b\in I_{\lambda_0}$. So $\dis X_{<b}\subseteq I_{\lambda_0}\subseteq \bigcup_{\lambda\in \Lambda}I_\lambda=J$. 
\end{proofenv}
...
\begin{propositionenv}
    ...
    \newline
    (2) If $X$ is infinite and $Y$ is finite,  then $X\setminus Y$ is infinite.(resp. uncountable)
\end{propositionenv}
\begin{proofenv}
    \quad
    \newline
    ...
    \newline
    or $(x, y)\in (Y\setminus X)^2$ and $h(x)=h(y)\Rightarrow x=y$.
    \newline
    or $x\in X, y\in Y\setminus X.h(x)\not=h(y)$ (So $h(x)=h(y)\Rightarrow x=y$).
    \newline
    or $y\in X, x\in Y\setminus X , h(x)\not=h(y)$.
    \newline
    (2)  Assume that $X\setminus Y$ is finite,  then $X=(X\setminus Y)\cup Y$ is also finite.
\end{proofenv}
\begin{notationenv}
...
\begin{proofenv}
    ...
    \newline
    If $f(n_0)<n_0$,  then $f(n_0)\notin A$,  and hence $f(n_0)=f(f(n_0))<f(n_0)$, contradiction. So $n_0<f(n_0)$.
    For any $n\in I$,  if $n_0\le n$ then $f(n_0)\le f(n)$. If $n_0>n$,  then $n\notin A$ and $f(n)=n<n_0$.$(*)$.
...
\end{propositionenv}
\begin{proofenv}
    We construct in a recursive way a family of elements in $I$. Let $x_0=\min(I)$. If $x_0, \dots , x_n$ are chosen (with $x_0<\dots<x_n$) we pick $x_{n+1}=\min(I\setminus\{x_0, \dots, x_n\})$. We slop at $N$ if $\{x_0, \dots, x_N\}=I$. Thus we obtain the increasing bijection needed by the proposition.
    \newline
...
\begin{proofenv}
    \quad
    \newline
    (1)$\Rightarrow$(2) We construct a sequence $(x_n)_{n\in \NN}$ in $X$ as follows. $X\not=\varnothing$. We pick arbitrary $x_0\in X$. Suppose that distinct elements $x_0, \dots,  x_n$ of $X$ are chosen. The set $X\setminus\{x_0, \dots, x_n\}\not=\varnothing$ since otherwise $X=\{x_0, \dots, x_n\}$ is finite. We pick $x_{n+1}\in X\setminus\{x_0, \dots, x_n\}, \,  x_0, \dots, x_{n+1}$ are distinct. The mapping $\NN\rightarrow X,  n\mapsto x_n$ is injective.
    \newline
...
\newline
(3)$\Rightarrow$(2) Let $g:X\rightarrow X$ be injective with $g(X)\not=X$. We pick $x_0\in X\setminus g(X)$. We define a sequence $(x_n)_{n\in\NN}$ by letting $x_{n+1}:=g(x_n)$. Since $g$ is injective,  $x_n\in g^n(X)\setminus g^{n+1}(X)$.
Otherwise $\exists y\in X$,  such that $x_n=g^{n}(x_0)=g^{n+1}(y)$. Hence $x_0=g(y)\in g(x)$ contradiction. Then $x_0, x_1, \dots, $ are distinct,  which defines an injective mapping $\NN\rightarrow X, n\mapsto x_n$.
\newline
