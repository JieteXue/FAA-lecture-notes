...
\begin{remark}
    ...
    \begin{enumerate}
        ...
        $$I(\lambda f)=\lambda I(f), \forall \lambda\in \RR\setminus\{0\}.$$
...
\begin{exampleenv}
    ...
    \newline
    (3) $B\setminus A=B\setminus\left(A\cap B\right)$. We may assume $A\subseteq B$. If $A=\interval[open left]{a}{b}$, $B=\interval[open left]{c}{d}$, $A\subseteq B$ implies $c\le a$, $b\le d$.
\end{exampleenv}
\begin{propositionenv}
    Let $\Omega$ be a set and $\mathcal{C}$ be a semiring on $\Omega$.
    \newline
    (1) Let $B\in \mathcal{C}$. Let $A_1,\cdots,A_n$ be sets in $\mathcal{C}$. Then $B\setminus \left(A_1\cup \cdots\cup A_n\right)$ can be written as the union of  a finite family of pairwise disjoint sets in $\mathcal{C}$.
    \newline
    ...
    In particular, $\forall (A,A')\in \mathcal{A}^2$, $\{A\cup A',A\cap A', A\setminus A'\}\subseteq \mathcal{A}$.
\end{propositionenv}
\begin{proofenv}
    \ \newline
    (1) We reason by induction on $n$. The case where $n=0$ is trivial.
    Suppose that $\dis B\setminus(A_1,\cup\cdots,\cup A_{n-1})=\bigcup_{i=1}^{n}C_i$, where $C_1,\cdots,C_m$ are pairwise disjoint sets in $\mathcal{C}$. Then 
    $$B\setminus(A_1,\cup\cdots,\cup A_{n})=\bigcup_{i=1}^{n}C_i\setminus A_n.$$
    Each $C_i\setminus A_n$ is of the form $\dis \bigcup_{j=1}^{d_i}D_{ij}$ with $D_{i,1},\cdots,D_{i,d_i}$  in $\mathcal{C}$, pairwise disjoint. So
    $$ B\setminus(A_1,\cup\cdots,\cup A_{n})=\bigcup_{i=1}^{n}\bigcup_{j=1}^{d_i}D_{ij}.$$
    \newline
    (2) Suppose that $\Theta=\{B_1,\cdots,B_n\}$. For any $i\in \{1,\cdots,n\}$, one has
    $$ B_i=\bigcup_{i\in J\subseteq\{1,\cdots,n\}}\left(\bigcap_{j\in J}B_j\right)\setminus\left(\bigcup_{k\in \{1,\cdots,n\}\setminus J} B_k\right).$$
    For any $J\subseteq\{1,\cdots,n\}$, $J\neq \varnothing$, we let
    $$ B_j\coloneq \left(\bigcap_{j\in J}B_j\right)\setminus \left(\bigcup_{k\in\{1,\cdots,n\}\setminus J}B_k\right)= \left(\bigcap_{j\in J}B_j\right)\bigcap \left(\bigcap_{k\in \{1,\cdots,n\}\setminus J} \complement_\Omega B_k\right).$$
...
\begin{propositionenv}
    ...
    (4) $I:S\longrightarrow \RR$, ($\RR$-linear mapping) that sends $\mathbbm{1}_A$, $A\in \mathcal{C}$ to $\mu(A)$ is an integral operator.
\end{propositionenv}
\begin{proofenv}
    \ \newline
    ...
    \end{align*}
    (2)$\Rightarrow$(3) Let $n\in\NN$, $B_n=A_n\setminus A_{n+1}\in \mathcal{A}$. Then $\mu(A_n)=\mu(B_n)+\mu(A_{n+1})$.
...
\begin{proofenv}
    ...
    $ B_n\setminus C_n=B_n\setminus \left(B_n\cap C_{n-1}\right)=B_n\setminus C_{n-1}\subseteq A_n\setminus C_{n-1}\subseteq A_{n-1}\setminus C_{n-1}$.
    $$ I_\varphi(\mathbbm{1}_{A_n\setminus C_n})=I_\varphi\left(\mathbbm{1}_{B_n\setminus C_n}\right)+I_\varphi\left(\mathbbm{1}_{A_n\setminus B_n}\right)\le I_\varphi\left(\mathbbm{1}_{A_{n-1}\setminus C_{n-1}}\right)+\frac{\varepsilon}{2^n}.$$
...
\begin{definitionenv}
    ...
    \newline
    (2) $A\in \mathcal{A}$, then $\Omega\setminus A\in \mathcal{A}$.
    \newline
    Note that (1) implies that $\varnothing\in \mathcal{A}$, in addition of (2), $\Omega\in \mathcal{A}$.
    $$ \bigcap_{i\in I}A_i=\Omega \setminus \left(\bigcup_{i\in I}\left(\Omega\setminus A_i\right)\right)\in \mathcal{A}.$$
...
\begin{proofenv}
    ...
    $$ f^{-1}\left(X\setminus A\right)=\Omega\setminus f^{-1}(A)\in \mathcal{A}.$$
    So, $X\setminus A\in \mathcal{A}'$. Thus $\mathcal{A}'$ is a $\sigma$-algebra, $\sigma(\mathcal{C})\subseteq \mathcal{A}'$.
\end{proofenv}
\begin{definitionenv}
...
\begin{proofenv}
    ...
    \newline
    (1) If $A\in \mathcal{A}$, $\mathbbm{1}_{\Omega\setminus A}=\mathbbm{1}_{\Omega}-\mathbbm{1}_{A}\in \mathcal{H}$.
    \newline
...
\begin{theoremenv}
    ...
    \begin{enumerate}
        \item If $(A,B)\in\mathcal{A}^2$, $B\setminus A\in \mathcal{A}$.
...
\begin{proofenv}
    ...
    \begin{enumerate}
        \item $$ \mathbbm{1}_{A\cap B}=\inf\{\mathbbm{1}_A,\mathbbm{1}_B\}\in \mathcal{L}^\uparrow.$$
            $$ \mathbbm{1}_{B\setminus A} =\mathbbm{1}_{B}-\mathbbm{1}_{A\cap B}\in \mathcal{L}^\uparrow (\text{by (1)}).$$
...
(4)
Let $$f_n = \sum_{k=1}^{n2^n-1} \frac{k}{2^n} \mathbbm{1}_{\{ \frac{k}{2^n} \leq f < \frac{k+1}{2^n} \}}+ n \mathbbm{1}_{\{ f \geq n \}}=\sum_{k=1}^{n2^n-1} \frac{k}{2^n} \mathbbm{1}_{A_{\frac{k+1}{2}}\setminus A_{\frac{k}{2^n}}} + n \mathbbm{1}_{B\setminus A}\in \mathcal{L}^\uparrow. $$
...
\begin{remark}
    ...
    In fact, if we let $B_n=A_n\setminus A_{n-1}, (n\ge 1)$, $B_0,\cdots,B_n$ are pairwise disjoint and $B_0=A_0$.
...
\begin{proofenv}
    ...
    If $(A,B)\in \mathcal{C}\times \mathcal{C}$, $\mu(A\cap B)\le \mu (A)$ ($\mu(A)=\mu(A\cap B)+\mu(A\setminus B)$).
    So $A\cap B\in \mathcal{C}$, $A\setminus B\in \mathcal{C}$.
\end{proofenv}
\begin{definitionenv}
    ...
    When $f\notin \mathcal{L}^1(\Omega,\mathcal{A},\mu)$. By convention, $\displaystyle \int_{\Omega} f \dd\mu\coloneq +\infty$.
\end{definitionenv}
\begin{propositionenv}
...
\begin{definitionenv}
    ...
    \newline
    (2) $\displaystyle\Omega=\bigcup_{n\in \NN}A_n$, then we say that $(\Omega,\mathcal{A}, \mu)$ is $\sigma$-finite.
\end{definitionenv}
\begin{remark}
    If $(\Omega,\mathcal{A},\mu)$ is $\sigma$-finite, then $\mathbbm{1}_{\Omega}\in \mathcal{L}^1(I)^\uparrow$.
\end{remark}
\begin{theoremenv}[Carath\'eodory]
...
\begin{theoremenv}
    ...
    \newline
    (1) There exists $\mu$-negligeable set $S\in \mathscr{P}(\Omega)$ such that $\forall x\in X\setminus A$, $f(x,\cdot): Y\longrightarrow \RR$, $y\longmapsto f(x,y)$ is continuous at $p$.
    \newline
    (2) For any $y\in Y$, $f(\cdot,y): X\longrightarrow \RR,\ x\longmapsto f(x,y)$ is $\mathcal{A}$-measurable.
    \newline
    (3) $\forall y\in Y$, $\exists A_y\in \mathscr{P}(\Omega)$ $\mu$-negligeable, such that $\forall x\in X\setminus A_y$, $|f(x,y)|\le g(x)$. Then $(y\in Y)\longmapsto \int_{X}f(x,y)\mu(\dd x)$ is continuous at $p$.
\end{theoremenv}
\begin{proofenv}
    ...
    On $X\setminus B$, $f_n$ converges pointwise to $f(\cdot,p)$ and $|f_n(x)|\le g(x)$.
    By dominated convergence theorem,
    $$ \lim_{n\rightarrow +\infty} \int_{X\setminus B} f(x,y_n)\mu(\dd x)=\int_{X\setminus B} f(x,p)\mu(\dd x).$$
\end{proofenv}
\begin{corollaryenv}
