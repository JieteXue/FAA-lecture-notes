...
\begin{proofenv}
    ...
    We claim that $L_{x} (V) \cap L_{y}(U) = \varphi$.
...
\begin{proofenv}
    ...
    \begin{enumerate}
        \item 
            ...
            By proposition \ref{thm:closure_pi_set}, $\varphi^{-1}(H) \supseteq \bar{H} \times$.
...
\begin{definitionenv}
    ...
    \begin{enumerate}
        ...
        \item If for any $(x,y) \in X$, there exists $g \in G$ such that $\varphi(g,x) = y$,
            ...
            and that $X$ is a \textbf{homogenous space} of $G$.
...
\begin{exampleenv}
    ...
    By definition, $\psi$ is transitive, hence $G/H$ is a homogenous space of $G$.
\end{exampleenv}
...
\begin{exampleenv}
    ...
    Let $O(V,\langle\ ,\ \rangle)$ be the orthogonal group of $(V,\langle\ ,\ \rangle)$ ($\RR$-linear isomorphism that one $\RR$-linear isometries).
    ...
    So $S$ is a homogenous of the group $O(V,\langle\ ,\ \rangle)$.
\end{exampleenv}
