...
\end{propositionenv}
\begin{proofenv}
    Without losing of generality, we may assume $I=\{1,\cdots,n\}$, $n\in \NN^+$. Let $\left(a_1,\cdots,a_n\right)\in K^n$ be such that 
...
\begin{definitionenv}
    Let $V$ be a vector space over $\RR$, $q: V\rightarrow \RR$ a quadratic form. If $q(V) \subseteq \RR_{\ge 0}$, we say that $q$ is \textbf{positive semi-definite}. If for any $x\in V\backslash\{0\}$, $q(x)>0$, then we say that $q$ is \textbf{positive definite}. (Resp. for negative.)
...
\end{lemmaenv}
\begin{proofenv}
    Without losing of generality, suppose that $q$ is positive semi-definite. By definition, the kernel of $q$ is contained in the isotropic cone of $q$.
